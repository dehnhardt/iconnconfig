...
void PortRemapWidget::midiPortRemapUpdated(PortRemapDirection direction) {
	switch (direction) {
    case PortRemapDirection::INPUT:
        m_pUpdateTimerInRemap->start(1000);
		break;
    case PortRemapDirection::OUTPUT:
...
void PortRemapWidget::createWidgets() {
    ...
    m_pInputRemapWidget =
        new PortRemapSettingsWidget(PortRemapDirection::INPUT, this);
    m_pOutputRemapWidget =
        new PortRemapSettingsWidget(PortRemapDirection::OUTPUT, this);
    m_pRemapDirectionTabWidget->addTab(m_pInputRemapWidget,
                                        tr("Input Remap"));
...
void PortRemapWidget::retrieveData() {
    ...
    getMidiPortRemap->setPortRemapDirection(PortRemapDirection::INPUT);
    getMidiPortRemap->setDebug(true);
    getMidiPortRemap->setCmdflags(0x40);
    m_pMidiPortRemapIn =
        static_cast<RetSetMidiPortRemap *>(getMidiPortRemap->query());
    getMidiPortRemap->setPortRemapDirection(PortRemapDirection::OUTPUT);
    m_pMidiPortRemapOut =
        static_cast<RetSetMidiPortRemap *>(getMidiPortRemap->query());
}
