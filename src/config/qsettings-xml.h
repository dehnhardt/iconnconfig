#ifndef QSETTINGSXML_H
#define QSETTINGSXML_H
#pragma once
...
static const QSettings::Format xmlFormat =
		QSettings::registerFormat("xml", &readSettingsXml, &writeSettingsXml);
static const QString rootName = "config";

// the following code is all used to save QSettings in xml. reading is
// accomplished
// through a stream-based xml parser. writing uses a stack-based approach to get
// all leaves of the tree-like QSettings structure right next to each other in
...
bool writeSettingsXml(QIODevice &device, const QMap<QString, QVariant> &map) {
	...
	//      to the stack to output them before closing the parent tag. the
	//      subsequent
